package com.piickr.zee.CustomClasses.model.service;
...
import com.piickr.zee.CustomClasses.model.data.SharedPreferenceHelper;
...
public class ServiceUtils {
    ...
    public static void updateUserStatus(Context context){
        if(isNetworkConnected(context)) {
            ...
            if (!uid.equals("")) {
                FirebaseDatabase.getInstance().getReference().child("profile/user/" + uid + "/status/isOnline").setValue(true);
                FirebaseDatabase.getInstance().getReference().child("profile/user/" + uid + "/status/timestamp").setValue(System.currentTimeMillis());
            }
        }
    }
    ...
    public static void updateFriendStatus(Context context, ListFriend listFriend){
        if(isNetworkConnected(context)) {
            for (Friend friend : listFriend.getListFriend()) {
                ...
                FirebaseDatabase.getInstance().getReference().child("profile/user/" + fid + "/status").addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        if (dataSnapshot.getValue() != null) {
                            ...
                            if ((boolean) mapStatus.get("isOnline") && (System.currentTimeMillis() - (long) mapStatus.get("timestamp")) > StaticConfig.TIME_TO_OFFLINE) {
                                FirebaseDatabase.getInstance().getReference().child("profile/user/" + fid + "/status/isOnline").setValue(false);
                            }
                        }
                    }
